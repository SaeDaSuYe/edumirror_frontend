...
export const analysisService = {
  ...
  async getDetailedAnalysisResult(sessionId: string): Promise<DetailedAnalysisResult> {
    try {
      ...
      // 분석 결과 조회 (analysisResult 관계를 통해)
      if (sessionData.analysisResult) {
        return {
          session_id: sessionId,
          overall_score: sessionData.analysisResult.overallScore || 0,
          detailed_scores: sessionData.analysisResult.detailedScores || {
            expression: 0,
            comprehension: 0,
            delivery: 0,
            engagement: 0
          },
          speech_analysis: sessionData.analysisResult.speechAnalysis || {
            script_adherence: 0,
          ...
          content_analysis: sessionData.analysisResult.contentAnalysis || {
            clarity: '분석 중',
            depth: '분석 중',
            relevance: '분석 중'
          },
          delivery_analysis: sessionData.analysisResult.deliveryAnalysis || {
            voice_quality: '분석 중',
            timing: '분석 중',
            engagement: '분석 중'
          },
          suggestions: sessionData.analysisResult.suggestions || [],
          analysis_completed_at: sessionData.analysisResult.createdAt || new Date().toISOString()
        };
      }
